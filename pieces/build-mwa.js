// Build MWA email templates — VISUAL MIRRORS of the Raise templates.
// Same blue header + footer, same blue page background, same purple emphasis.
// The ONLY thing that changes vs Raise: the logo (MWA wordmark instead of
// the Raise icon) and footer copy (mentions MWA Cambridge).

const LOGO_URI = await readFile('ui_kits/email_templates/pieces/_mwa-logo-data-uri.txt');
const FONT = `'Nunito Sans', 'Helvetica Neue', Helvetica, Arial, sans-serif`;

const wrap = ({ title, eyebrow, body, footerEmail = null }) => `<!DOCTYPE html>
<html>
<head>
<meta charset="utf-8">
<title>${title}</title>
...
<table role="presentation" width="580" cellpadding="0" cellspacing="0" style="background-color:#FAFAFA; border-radius:12px; overflow:hidden; box-shadow:0 2px 16px rgba(99,200,212,0.15);">
  ...
  <!-- Header: blue (same as Raise), centred MWA wordmark logo, optional eyebrow -->
  <tr>
    <td align="center" style="background-color:#63C8D4; padding:28px 40px 24px 40px;">
      <img src="${LOGO_URI}" width="200" height="100" alt="May Week Alternative - A Celebration of Giving" style="display:block; border:0; outline:none; text-decoration:none; width:200px; height:100px;">
  ...
  <!-- Four-colour bar (sacred order, shared with Raise) -->
  <tr>
...
// ----- 2. CHAPTER COMMS -----
const commsTpl = wrap({
  title: 'MWA - Internal comms',
  eyebrow: null,
  body: `
  <tr>
    <td style="padding:32px 40px 32px 40px;">
      ${p('Hey <strong>[NAME]</strong> &#x1F44B;')}
      ${p('Hope your week is going well! Just a quick update from the MWA Cambridge team.')}
      ${p('[Replace this paragraph with your message. Add more paragraphs as needed.]')}
      ${signoff('[Your name]', '[Your role]')}
    </td>
  </tr>`,
...
const reimbursementTpl = wrap({
  title: 'MWA - Reimbursement Confirmation',
  ...
  <tr>
    <td style="padding:0 40px 28px 40px;">
      ...
      ${p('Thanks for everything you do for MWA - we really appreciate it.', 0)}
    </td>
  </tr>
  <tr>
...
const partnerTpl = wrap({
  ...
  <tr>
    <td style="padding:32px 40px 8px 40px;">
      ...
      ${p('Hope you&rsquo;re well. I&rsquo;m writing on behalf of <strong>May Week Alternative (MWA)</strong>, the Cambridge chapter of Raise: A Celebration of Giving &mdash; a student-led fundraising charity (number 1202899).')}
...
const eventInviteTpl = wrap({
  ...
  ${callout(`
              ...
              ${detailRow('Who&rsquo;s coming', '[Audience - committee, donors, partners, etc.]')}
...
const impactReportTpl = wrap({
  title: 'MWA - Impact this year',
  eyebrow: null,
  body: `
  <tr>
    <td style="padding:28px 40px 8px 40px;">
      ${p('Hi <strong>[NAME]</strong>,')}
      <p style="margin:0 0 20px 0; font-family:${FONT}; font-size:22px; font-weight:700; line-height:1.3; color:#5D215E;">Cambridge did something extraordinary this year.</p>
    </td>
  </tr>
  <tr>
    <td style="padding:0 40px 24px 40px;">
            ...
            <table role="presentation" width="100%" cellpadding="0" cellspacing="0">
              <tr>
                <td align="center" width="33%" style="padding:6px;">
                  ...
                  <p style="margin:6px 0 0 0; font-family:${FONT}; font-size:11px; text-transform:uppercase; letter-spacing:1px; color:#888888;">Colleges</p>
                </td>
  ...
  <tr>
    <td style="padding:0 40px 12px 40px;">
      ...
      ${p('[Optional second paragraph - a specific moment, a beneficiary story, a committee highlight.]')}
      ${cta('[Button text]', '[LINK]', 'purple')}
    </td>
  </tr>
  <tr>
...
const templates = {
  'mwa_01_base.html':               baseTpl,
  'mwa_02_chapter_comms.html':      commsTpl,
  'mwa_03_reimbursement.html':      reimbursementTpl,
  'mwa_04_internal_programme.html': announcementTpl,
  'mwa_05_partner_external.html':   partnerTpl,
  'mwa_06_event_invite.html':       eventInviteTpl,
  'mwa_07_impact_report.html':      impactReportTpl,
};
...
log('Done. ' + Object.keys(templates).length + ' MWA templates regenerated as Raise visual mirrors.');
